$(document).ready(function(){
    ...
    const ReadData = async () => {
        ...
        const url = "functions/php/vendor/GetData.php";
        await fetch(url,{
            method : "GET",
        })
        .then(response => {
        ...
        .then(response => {
            ...
            document.getElementById('targetDataVendor').innerHTML = vendor;
    
            // Event ketika Tombol 
            const btn_ubah = document.querySelectorAll('#btn-ubah_vendor');
            btn_ubah.forEach(el => {
                el.addEventListener('click', function() {
                    const id_vendor = this.dataset.id;
                    document.location.href = `?p=vendor/ubah&data=${id_vendor}`;
                })
            })

            // Event ketika tombol hapus di klik 
            const btn_del = document.querySelectorAll('#btn-hapus_vendor');
    ...
    const hapusData = (id_vendor) => {
        ...
        $.ajax({
            type : "POST",
            url : "functions/php/vendor/HapusData.php",
        ...
        return `
            <tr>
                ...
                <td class="flex" width="10%">
                    <button class='btn btn-outline-primary btn-sm btn-edit' data-id="${response[i].id_vendor}" id="btn-ubah_vendor">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                        <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                        <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                    </svg>
                    </button>
                    <button class='btn btn-outline-danger btn-sm btn-hapus' data-id="${response[i].id_vendor}" id='btn-hapus_vendor'>
